...
public class ExpandListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private OnListRefreshListener mOnRefershListener;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE :
                ...
                // 如果: -头布局的高度 > paddingTop的值 (说明正在上拉) 执行super.onTouchEvent(ev);
                if (firstVisibleItemPosition == 0 && -headerViewHeight < paddingTop) {//正在下拉
                    if (paddingTop > 0 && currentState == DOWN_PULL_REFRESH) { // 下拉到完全显示了.
                        Log.i(TAG, "松开刷新");
